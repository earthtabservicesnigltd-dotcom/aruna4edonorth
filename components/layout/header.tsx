"use client";

import Link from "next/link";
import Image from "next/image";
import { usePathname } from "next/navigation";
import { useState } from "react";
import { Menu, X, Coins } from "lucide-react";
import { Button, buttonVariants } from "@/components/ui/button";

import {
  Sheet,
  SheetContent,
  SheetHeader,
  SheetTitle,
  SheetTrigger,
  SheetClose,
} from "@/components/ui/sheet";
import { mainNav, siteConfig } from "@/lib/site-config";
import { cn } from "@/lib/utils";

const SOCIAL_LINKS = [
  { label: 'Facebook',  icon: 'M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z' },
  { label: 'X',         icon: 'M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.744l7.73-8.835L1.254 2.25H8.08l4.259 5.63 4.905-5.63zm-1.161 17.52h1.833L7.084 4.126H5.117z' },
  { label: 'Instagram', icon: 'M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z' },
  { label: 'YouTube',   icon: 'M23.498 6.186a3.016 3.016 0 00-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 00.502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 002.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 002.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z' },
]

export function Header() {
  const pathname = usePathname();
  const [open, setOpen] = useState(false);

  const isActive = (href: string) =>
    href === "/" ? pathname === "/" : pathname.startsWith(href);

  return (
    <nav className="fixed top-0 inset-x-0 z-50 bg-white border-b border-ink/8 shadow-sm">
      <div className="max-w-site mx-auto px-3 h-[72px] flex items-center justify-between">
        {/* Brand */}
        <Link href="/" className="flex items-center gap-2.5 min-w-0">
          <Image
            src="/images/logo.png"
            alt="Campaign logo"
            width={40}
            height={40}
            className="h-10 w-auto shrink-0 rounded-full"
          />
          <small className="font-mono text-[10px] text-orange tracking-widest uppercase whitespace-nowrap">
            {siteConfig.tagline}
          </small>
        </Link>

        {/* Desktop nav — hidden on mobile */}
        <ul className="hidden lg:flex items-center gap-1">
          {mainNav.map((item) => (
            <li key={item.href}>
              <Link
                href={item.href}
                className={cn(
                  "font-body text-[14.5px] px-3.5 py-2 rounded-site border-b-2 transition-colors",
                  isActive(item.href)
                    ? "text-orange font-semibold border-orange"
                    : "text-ink border-transparent hover:bg-paper hover:text-orange"
                )}
              >
                {item.title}
              </Link>
            </li>
          ))}
        </ul>

        {/* Mobile menu */}
        <Sheet open={open} onOpenChange={setOpen}>
          <SheetTrigger 
            className={cn(
              "lg:hidden",
              buttonVariants({ variant: "secondary", size: "icon" })
            )}
            aria-label="Toggle navigation"
          >
            {open ? <X className="w-5 h-5" /> : <Menu className="w-5 h-5" />}
          </SheetTrigger>


          <SheetContent
            side="right"
            className="w-[min(87vw,360px)] bg-ink text-white border-none p-0 flex flex-col h-full"
          >
            <SheetHeader className="border-b border-white/12 px-6 py-5 text-left shrink-0">
              <SheetTitle className="text-white font-display text-lg">
                <Image
                  src="/images/logo.png"
                  alt="Home"
                  width={55}
                  height={55}
                  className="mb-2 rounded-full"
                />
                <span className="font-mono text-[11px] tracking-widest uppercase text-orange block">
                  {siteConfig.tagline}
                </span>
              </SheetTitle>
            </SheetHeader>

            {/* Scrollable nav area */}
            <div className="flex-1 overflow-y-auto">
              <ul>
                {mainNav.map((item) => (
                  <li key={item.href} className="border-b border-white/6">
                    <Link
                      href={item.href}
                      onClick={() => setOpen(false)}
                      className={cn(
                        "block px-6 py-4 text-base transition-all hover:pl-8",
                        isActive(item.href)
                          ? "text-orange font-semibold bg-orange/10"
                          : "text-white/80 hover:text-white"
                      )}
                    >
                      {item.title}
                    </Link>
                  </li>
                ))}
                <li className="border-b border-white/6">
                  <Link
                    href="/abubakari-institute"
                    onClick={() => setOpen(false)}
                    className="block px-6 py-4 text-base text-white/80 hover:text-white hover:pl-8"
                  >
                    Abubakari Aruna Institute
                  </Link>
                </li>
              </ul>
            </div>

            {/* Fixed bottom section */}
            <div className="shrink-0 px-5 pb-6 flex flex-col gap-3">
              <Link
                href="/donate"
                onClick={() => setOpen(false)}
                className="flex items-center justify-center gap-2 w-full py-3.5 rounded-full border border-orange/40 text-orange font-semibold text-sm hover:bg-orange/10 transition-colors"
              >
                <Coins className="w-4 h-4" />
                DONATE TO THE CAMPAIGN
              </Link>

              <div className="flex justify-center gap-3 mt-2">
                {SOCIAL_LINKS.map(s => (
                  <a key={s.label} href="#" aria-label={s.label} className="text-white/50 hover:text-[#f97316] transition-colors">
                    <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24">
                      <path d={s.icon} />
                    </svg>
                  </a>
                ))}
              </div>

              <p className="text-center font-mono text-[10.5px] text-white/50 leading-relaxed mt-4 pt-3 border-t border-white/10">
                Constituency Office · Auchi, Edo State
                <span className="block mt-1 font-bold text-orange uppercase">
                  People First. Progress Always.
                </span>
              </p>
            </div>
          </SheetContent>

        </Sheet>
      </div>
    </nav>
  );
}