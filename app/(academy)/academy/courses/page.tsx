...
export default function SchoolsPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (user) {
        const { data: stu } = await supabase.from("students").select("id, programme_id").eq("email", user.email).single();
        setStudentProgrammeId(stu?.programme_id || null);

        // 2. Get All Schools & Courses
        const { data: progs } = await supabase.from("programmes").select("id, name, icon, blurb, cert, courses(id, title, order)").eq("active", true);
        setSchools(progs || []);

        // 3. Get Student Progress (Using student.id, NOT user.id)
        if (stu) {
          const { data: progData } = await supabase.from("student_progress").select("course_id, status").eq("student_id", stu.id);
